...
export default function find(query = {}, pagination) {
  return function(dispatch) {
    const Collection = lore.collections.imageBookmark;
    ...
    setTimeout(function() {
      // set the fake data
      collection.set(db.image_bookmarks, { parse: true });
      ...
      const actions = normalize(lore, 'imageBookmark').collection(collection);

      dispatch({
        type: ActionTypes.fetchPlural('imageBookmark'),
